...
public:
    ...
    bool isPlayerHalf(GridPos pos) const;
    bool isEnemyHalf(GridPos pos) const;

    std::optional<UnitId> occupant(GridPos pos) const;
    bool empty(GridPos pos) const;
    bool place(UnitId unitId, GridPos pos);
    void remove(GridPos pos);
    void move(GridPos from, GridPos to);
    void clearEnemies();
    void clearAll();
...
class Pathfinder {
public:
    std::vector<GridPos> findPathToAttackRange(
        const Board& board,
        GridPos start,
        GridPos target,
        int attackRange
...
private:
    std::vector<GridPos> neighbors(GridPos pos) const;
};
```
...
如果希望更符合格子战棋，也可把攻击距离改为曼哈顿距离；但 PA 写明索敌使用欧氏距离，因此目标排序建议使用欧氏距离平方。
...
struct DragState {
    ...
    UnitId unitId = 0;
    int sourceSlot = -1;
    std::optional<GridPos> sourcePos;
...
5. 阶段二先使用 BFS 离散格子移动，足以满足阻挡和防重叠要求，复杂度低于连续坐标碰撞。
...
6. 加入 `CombatSystem`，先实现站桩攻击，再加入移动和 BFS。
7. 加入技能多态。
8. 加入商店、金币、人口。
9. 加入羁绊、升星、装备。

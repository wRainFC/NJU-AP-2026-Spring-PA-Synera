...
UI 使用 Raylib 绘制。窗口由 `GameWindow` 管理，内部固定虚拟画布为 `1280 x 720`，真实窗口变化时通过 letterbox 缩放显示，并将鼠标坐标投影回虚拟画布。
...
在验收时，可以重点解释 `CombatSystem` 的状态机、`Pathfinder` 的路径搜索、`SynergySystem` 的羁绊结算，以及 `SaveSystem` 的序列化恢复过程。

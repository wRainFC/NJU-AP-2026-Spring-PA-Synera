...
- 项目阶段完成度：
  ...
  - 阶段二：已完成
  - 阶段三：已完成
  - 阶段四：已实现高级经济系统，并补充可缩放窗口、贴图 fallback 和单位 sprite-sheet 动画支持
...
- `GameApp`：应用主循环，负责调度输入命令、规则系统更新和渲染。
- `GameWindow`：封装 Raylib 窗口、虚拟画布和鼠标坐标投影，支持窗口缩放后的稳定命中测试。
- `InputController`：将鼠标点击、拖拽和结算面板操作转换为 `InputCommand`，并向渲染层提供选中单位与拖拽预览读模型。
...
`EconomySystem` 在回合结算时统一计算收益，并更新玩家连胜/连败状态。金币收益由三部分组成：

- 基础奖励：胜利 +6，失败 +3。
- 利息：按结算前金币计算，每 10 金币 +1，最多 +5。
- 连胜/连败奖励：连续 2/4/6 场及以上分别 +1/+2/+3。

顶部状态栏会显示当前 streak 和预计利息；回合结算面板会展示金币变化和 Base / Interest / Streak 收益明细。验收时可以通过保留金币、连续胜利或连续失败观察额外收益变化。
...
assets/
  ...
  data/combat_animations.json
  fonts/minecraftia.fnt
  fonts/minecraftia.png
  fonts/ui.fnt       可选 fallback
  fonts/ui.ttf       可选 fallback
  fonts/ui.otf       可选 fallback
  textures/
    ...
    ui/sell_area.png
    ui/panel.png
    actions/<action_clip>.png
    projectiles/basic.png
    projectiles/<projectile_clip>.png
    effects/hit.png
    effects/death.png
    effects/<impact_clip>.png
...
字体会优先加载 `assets/fonts/minecraftia.fnt`。该文件使用 Raylib 支持的 AngelCode BMFont 文本格式，并引用同目录下的 `minecraftia.png` 字形贴图；贴图字形为白色、透明背景，加载后使用 `TEXTURE_FILTER_POINT` 以保持像素风显示。如果该字体不存在或格式不受支持，加载顺序会继续 fallback 到 `assets/fonts/ui.fnt`、`assets/fonts/ui.ttf`、`assets/fonts/ui.otf`，最后使用 Raylib 默认字体。构建 `synera` 目标时会先清理再复制 `assets/` 到运行目录，避免替换字体或贴图后继续读到旧资源。单位动画支持单行 sprite sheet，帧宽等于图片高度，帧数为 `image_width / image_height`。

战斗 action 和动画表现分成两个 manifest。`assets/data/combat_actions.json` 描述逻辑 profile，`durationSeconds`、`hitTimes` 控制 CombatSystem 的命中结算，并通过 `animationProfileId` 引用表现配置。`assets/data/combat_animations.json` 描述动画 profile，`unitClip`、`projectileClip`、`castImpactClip`、`damageImpactClip`、`healImpactClip`、`statusImpactClip` 控制 UI 贴图、帧数和 FPS。manifest 缺失或贴图缺失时会 fallback 到内置默认 profile、旧的单位状态动画或几何效果。
...
1. UI 架构拆分
   ...
   AI 辅助分析了原始 `Renderer` 过重的问题，并提出将窗口管理、输入投影、资源加载、通用绘制、棋盘绘制和单位绘制拆成独立模块。最终实现为 `GameWindow`、`InputController`、`UiState`、`UiDrawing`、`GridItem`、`UnitItem` 和 `RenderAssets`。其中 `InputController` 只产生输入命令和 UI 读模型，`GameApp` 再统一调用战斗、商店、羁绊等规则系统，避免 UI 层直接散落规则修改。

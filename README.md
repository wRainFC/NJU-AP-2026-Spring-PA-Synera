...
战斗攻击动画由 `assets/data/combat_actions.json` 描述。每个 action profile 同时提供逻辑命中时间和视觉 clip：`durationSeconds`、`hitTimes` 控制 CombatSystem 的命中结算，`unitClip`、`projectileClip`、`impactClip` 控制 UI 贴图、帧数和 FPS。manifest 缺失或贴图缺失时会 fallback 到内置默认 profile、旧的单位状态动画或几何效果。
